...
public class ClientRPL implements Runnable {
     ...
     */
    public ClientRPL(Socket clientSocket, InputUser inputUser, OutputUser outputUser, PileRPL pile, boolean isSharedMode, Lock pileLock) {
        this.clientSocket = clientSocket;
        this.inputUser = inputUser;
        this.outputUser = outputUser;
        this.pile = pile;
        this.isSharedMode = isSharedMode;
        this.pileLock = pileLock;
    ...
    private void processInput(String arg) throws IOException {
        ...
        if (arg.equalsIgnoreCase("exit")) {
            ...
            if (isSharedMode) { // If the mode is shared I need to lock the stack to avoid concurrency
                pileLock.lock();
                try {
                    pile.push(vecteur);
                    outputUser.sendOutput(pile.toString().trim());
                } finally {
                    pileLock.unlock(); // Then I unlock the stack because I have done my operation.
                }
            } else {
                pile.push(vecteur);
                outputUser.sendOutput(pile.toString().trim());
